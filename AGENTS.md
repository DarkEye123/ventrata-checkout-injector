...
- Native Chrome context-menu items cannot be reliably shown only for exact DOM targets inside `ventrata-checkout-element`, especially through shadow DOM. Keep the current fallback: always show the extension menu on page right-click and let the content script decide whether the last right-click happened inside checkout before copying configuration.
